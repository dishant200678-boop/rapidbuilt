...
export class ProjectService {
  static async createProject(data: any, user: TokenPayload): Promise<IProject> {
  ...
  static async getProjects(params: {
    ...
    sortBy?: string;
    sortOrder?: 'asc' | 'desc';
  }): Promise<{ projects: IProject[]; total: number; page: number; limit: number; totalPages: number }> {
    ...
    if (params.riskCategory) filter.riskCategory = params.riskCategory;
  ...
  static async updateProject(id: string, updates: any, user: TokenPayload): Promise<IProject> {
    if (!Types.ObjectId.isValid(id)) {
      throw new AppError(404, 'NOT_FOUND', 'Project not found');
    }
  ...
  static async deleteProject(id: string, user: TokenPayload): Promise<void> {
    if (!Types.ObjectId.isValid(id)) {
      throw new AppError(404, 'NOT_FOUND', 'Project not found');
    }
