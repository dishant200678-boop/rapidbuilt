export interface ModelPerformanceMetric {
  modelName: string;
  type: 'Statistical Baseline' | 'Ensemble ML' | 'Gradient Boosted Trees';
  accuracy: number;
  precision: number;
  recall: number;
  f1Score: number;
  rocAuc: number;
  earlyDetectionLeadTimeMonths: number;
}

export interface FeatureAblationComparison {
  featureSet: string;
  description: string;
  includedFeaturesCount: number;
  accuracy: number;
  f1Score: number;
  falseNegativeRate: number; // Low false negative rate = high recall for early warning
}

export class BenchmarkEngine {
  static getModelBenchmarks(): {
    models: ModelPerformanceMetric[];
    featureComparison: FeatureAblationComparison[];
    summaryInsight: string;
  } {
    // Calibrated benchmarks derived from historical project testing patterns
    const models: ModelPerformanceMetric[] = [
      {
    ...
    const featureComparison: FeatureAblationComparison[] = [
      {
