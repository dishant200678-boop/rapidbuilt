import { IProject, IShapFactor, RiskCategory } from '../projects/project.model.js';

export interface EvaluatedRiskMetrics {
  progressGap: number;
  costGrowthPct: number;
  burnRate: number;
  expenditureVsProgressDiscrepancy: number;
  riskScore: number;
...
export class RiskEngine {
   ...
   */
  static evaluateProjectRisk(project: Partial<IProject>): EvaluatedRiskMetrics {
    ...
    const delayProbability = Math.min(
      99,
      Math.max(5, Math.round(riskScore * 0.95 + (progressGap > 20 ? 8 : 0)))
    );

    // Cost Overrun Probability is strongly linked to cost growth and expenditure discrepancy
    const costOverrunProbability = Math.min(
      98,
      Math.max(
        5,
        Math.round(
          (costGrowthPct > 0 ? 50 : 10) +
