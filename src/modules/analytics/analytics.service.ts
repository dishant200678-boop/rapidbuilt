...
import { IProject } from '../projects/project.model.js';

export interface WhatIfSimulationInput {
  additionalBudgetCr?: number;
  physicalProgressBoostPct?: number;
  resolvedMilestonesCount?: number;
  accelerateContractor?: boolean;
}

export class AnalyticsService {
  static async explainProjectRisk(projectId: string): Promise<any> {
  ...
  static async simulateWhatIf(projectId: string, input: WhatIfSimulationInput): Promise<any> {
    const project = await ProjectService.getProjectById(projectId);
    ...
    const simulatedProject: Partial<IProject> = {
      originalCost: project.originalCost,
