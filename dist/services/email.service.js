...
export class EmailService {
    static async sendEarlyWarningEmail(to, projectTitle, riskScore, reason) {
        if (!transporter) {
            logger.info(`[Email Mock] Early warning alert for "${projectTitle}" (Risk: ${riskScore}) logged (SMTP not configured)`);
            return;
        }
        if (env.NODE_ENV === 'production') {
            ...
            logger.info(`Early warning email skipped in ${env.NODE_ENV} environment`);
        }
    }
}
